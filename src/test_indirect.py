...
cal = fm_cal('../test/test_indirect_config.ini')
cal.initialize()
...
numx = len(x)
q = np.arange(550.0, 800.0, 50.0)
numy = len(q)
X, Y = np.meshgrid(x, q, indexing='ij')
tcd ={}
tcdind = {}
tcount = 0
...
for cdind0, cd0 in enumerate(x):
    tcd[0] = cd0
    tcdind[0] = cdind0
    for qind, q1 in enumerate(q):
        # tcd[1] = cd1
        # tcdind[1] = cdind1
        numind = len(tcdind)
        # simres = cal.update_var(cdind0, cd0, cdind1, cd1)
        simres = cal.update_var2(tcount, tcd, q1)
        tmpz[cdind0,qind] = simres.loc[tcount, 'rmse']
